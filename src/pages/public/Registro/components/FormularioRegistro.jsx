    ...
    <form onSubmit={manejarEnvio} className="mt-6 flex flex-col gap-4">
      ...
      <label className="flex items-center gap-3 text-sm text-[#aaa5b0]">
        <div className={`relative h-6 w-11 shrink-0 rounded-full transition-colors duration-300 ${
            aceptaTerminos ? 'bg-blue-500' : 'bg-white/20'
          }`}>
          <button type="button"
            ...
            className={`absolute top-0.5 left-0.5 h-5 w-5 cursor-pointer rounded-full bg-white shadow-md transition-transform duration-300 ${
              aceptaTerminos ? 'translate-x-5' : 'translate-x-0'
            }`}
          />
        </div>
      ...
      <button type="submit" className="h-11 rounded-lg bg-gradient-to-r from-[#40a9e6] to-[#3d7de3] font-bold text-white transition hover:brightness-110">
        Crear cuenta
      </button>
